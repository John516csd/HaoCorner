...
export function PhotoModal({ isOpen, onClose, folderTitle, photos }: PhotoModalProps) {
  ...
  const photoPreview = (
    <AnimatePresence>
      {selectedPhoto && selectedPhotoIndex !== null && (
        <motion.div
          className="fixed inset-0 z-[130] flex cursor-zoom-out items-center justify-center overflow-hidden overscroll-contain bg-[#faf9f6]/90 px-4 py-8"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          >
            <div
              className="relative max-w-full overflow-hidden bg-[#f8fafc]"
              style={{
                aspectRatio: `${selectedPhotoSize.width} / ${selectedPhotoSize.height}`,
                width: `min(calc(92vw - 1.5rem), calc(1120px - 1.5rem), calc(78vh * ${selectedPhotoRatio}))`,
              }}
            >
              <img
                src={selectedPhoto}
  ...
  return (
    <>
      ...
      {typeof document !== "undefined" ? createPortal(photoPreview, document.body) : null}
    </>
  );
}

...
export default function Collection({ visible, fallback, onScroll, onHover, onChoose }: {
  ...
  return <div className={styles.collection} data-collection-scroll data-visible={visible} {...(!visible ? { 'aria-hidden': true, inert: true } : {})} onScroll={onScroll}>
    ...
    <h1 className={styles.title}>{t("唱片收藏室")}</h1>
    <div className={styles.boxes}>
      {entries.map((entry, index) => { const artist = musicArtists.find(artist => artist.id === entry.artistId)!; return <Link key={entry.id} href={`/${artist.id}`} scroll={false}
        ...
        className={styles.artist} aria-label={t('打开 {artist} 的收藏箱，{count} 张专辑', { artist: artistDisplayName(artist, locale), count: artist.albums.length })}
        ...
        <div className={styles.caption}>
          ...
          <div className={styles.artistName}><h2>{artistDisplayName(artist, locale)}<ArrowUpRight size={23} strokeWidth={1.3} /></h2><p>{locale === 'en' ? artist.name : artist.english}</p></div>
          <span className={styles.count}>{t('{count} 张专辑', { count: artist.albums.length })}<small>{artist.albums[0].year} — {artist.albums.at(-1)!.year}</small></span>
        </div>
      </Link>; })}
    </div>
    <div className={styles.walkHint}><span><ArrowDown size={13} />{t("向前滚动")}</span><span><b data-walk-position>01</b> / <span data-walk-total>{String(entries.length / 2).padStart(2, '0')}</span></span></div>
    </div></div>
  </div>;
}
